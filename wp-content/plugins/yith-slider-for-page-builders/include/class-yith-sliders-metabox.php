...
class YITH_Sliders_Metabox {
	...
	public function generate_fields( $post ) {
		...
		if ( 'yith_slider' == $current_screen->post_type && 'add' != $current_screen->action ) {
			...
			foreach ( $children as $child ) {
				...
				$thumbnail_link = '<a href="' . $edit_link . '"><span class="slide-id">' . $id . '</span>' . $thumbnail_img . '</a>';

				$output .= '<li data-order="">';
				$output .= $thumbnail_link;
	...
	public function add_slider_controls( $post ) {
		...
		$slider_min_height = get_post_meta( $post->ID, 'yith_slider_control_min_heigth', true );
		if ( ! $slider_min_height || '' === $slider_min_height ) {
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<?php esc_html_e( 'Slider content min height', 'yith-slider-for-page-builders' ); ?>
					<small><?php esc_html_e( 'Default value: ', 'yith-slider-for-page-builders' ); ?> 200 px</small>
				</th>
				<td>
					<p>
						<label class="form-slider">
							<input type="range" min="50" max="1900" value="<?php echo esc_attr( $slider_min_height ); ?>" class="slider" step="10" id="yith_slider_control_min_heigth" name="yith_slider_control_min_heigth">
							<span id="container_min_height"></span>
						</label>
					</p>
				</td>
			</tr>
	...
	public function save_slider_controls( $post_id ) {
		...
		if ( isset( $_POST['yith_slider_control_min_heigth'] ) ) {
			update_post_meta( $post_id, 'yith_slider_control_min_heigth', esc_attr( intval( wp_unslash( $_POST['yith_slider_control_min_heigth'] ) ) ) );
		}
