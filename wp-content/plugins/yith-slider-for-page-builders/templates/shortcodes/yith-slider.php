...
function yith_slider_for_page_builders_slider_sc( $atts ) {
	...
	$slide_container_min_height = get_post_meta( $atts['slider'], 'yith_slider_control_min_heigth', true );
	if ( '' !== $slide_container_min_height ) {
		$slider_style .= 'min-height: ' . $slide_container_min_height . 'px; ';
	}
	...
	// Slider option.
	$transition_type  = get_post_meta( $atts['slider'], 'yith_slider_control_animation_type', true ) == 'fade' ? 'true' : 'false';
	$autoplay         = get_post_meta( $atts['slider'], 'yith_slider_control_autoplay', true ) == 'autoplay' ? 'true' : 'false';
	$autoplay_timing  = get_post_meta( $atts['slider'], 'yith_slider_control_autoplay_timing', true );
	$infinite_sliding = get_post_meta( $atts['slider'], 'yith_slider_control_infinite_sliding', true ) == 'infinite-sliding' ? 'true' : 'false';
		...
		<script>
			jQuery( function( $ ) {
					$('.yith-slider-<?php echo esc_attr( $atts['slider'] ); ?>').slick({
						...
						infinite: <?php echo esc_attr( $infinite_sliding ); ?>,
						arrows: <?php echo esc_attr( $arrow_nav ); ?>,
						prevArrow: '<button type="button" class="yith-slider-nav slide-prev <?php echo esc_attr( $arrow_nav_style ); ?>">Previous</button>',
						nextArrow: '<button type="button" class="yith-slider-nav slide-next <?php echo esc_attr( $arrow_nav_style ); ?>">Next</button>',
						centerMode: <?php echo esc_attr( $center_mode ); ?>,
						dots: <?php echo esc_attr( $dots_nav ); ?>,
						dotsClass: 'yith-slider-dots <?php echo esc_attr( $dots_nav_style ); ?>',
						autoplaySpeed: <?php echo esc_attr( $autoplay_timing ); ?>,
					});
				});
		</script>
		<?php
